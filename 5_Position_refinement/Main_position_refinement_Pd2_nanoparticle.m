...
clc
addpath('./src/')
inputDir = './Input/Pd2_nanoparticle/';
...
model=importdata([inputDir 'atomic_model_Pd2_particle.mat']);
...
Z_arr = [46]; % atomic Z number of Ta
Res = 0.454; % indicate the pixel size for measured projections
...
% save([inputDir 'B2scan_' num2str(iiiiii) '.mat'],'y_pred','para0','errRscan','err_arr','err_arr2')

% optimized around the above best B factor by gradient descend
xdata.step_sz    = 1;
xdata.iterations = 10;
[y_pred,para0,errR] = gradient_B(para0, xdata, projections);
% save([inputDir 'B2scan_' num2str(iiiiii) '_itr10.mat'],'y_pred','para0','errR')
...
% save([inputDir 'B2xyz_' num2str(iiiiii) '_itr100.mat'],'y_pred','model_arr','errR')
end
save('./Output/Final_atomic_model_Pd2_nanoparticle.mat','model_refined')

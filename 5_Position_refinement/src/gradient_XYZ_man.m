function [Projs,params,errR] = gradient_XYZ_man(para, xdata, ydata)
...
if size(para,2)==numel(unique(para(1,:)))
    for k=1:numel(unique(para(1,:)))
        h(atom==k) = para(1,k);
        b(atom==k) = para(2,k);
    end
...
for iter=1:iterations
    dt = (num_pj*step_sz(iter) / (8*sum_R2*num_pj*(N1+N2)/2) ) ./ ( h.^2 .* b.^2 );

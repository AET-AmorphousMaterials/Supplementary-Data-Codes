function [Projs,param,errR] = gradient_HB(para, xdata, ydata)
errR=[];
...
% for j=1:atom_type_num
%     grad_b_new(j) =  -sum(sum(sum( res.*grad_b(:,:,:,j) )));
%     grad_h_new(j) =   sum(sum(sum( res.*grad_h(:,:,:,j) )));
%     if iter == 1
% %         dt_b(j) = -(t(j)/max_fa^2 /N1^2/N_s^6);
% %         dt_h(j) =  (t(j)/max_fa^2 /N1^2);
%         
%         dt_b(j) = 0.001/abs(grad_b_new(j));
%         dt_h(j) = 0.001/abs(grad_h_new(j));        
% 
%     elseif iter<=30
%         dt_b(j) = -dt_b(j)*grad_b_old(j)/(grad_b_new(j) - grad_b_old(j));
%         dt_h(j) = -dt_h(j)*grad_h_old(j)/(grad_h_new(j) - grad_h_old(j));
%     end
%     b(j) = b(j) - dt_b(j)*grad_b_new(j);
%     h(j) = h(j) - dt_h(j)*grad_h_new(j);
% end
% grad_b_old = grad_b_new;
% grad_h_old = grad_h_new;

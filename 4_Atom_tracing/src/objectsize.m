function [objx,objy,objz]=objectsize(dimx,dimy,dimz,custom_euler_rot_vecs,angles)
objz=dimz;
npj=size(angles,1);
dxarr=zeros(1,npj);
dyarr=zeros(1,npj);
...
for i=1:npj
    R = (MatrixQuaternionRot(custom_euler_rot_vecs{1}, angles(i,1)) * MatrixQuaternionRot(custom_euler_rot_vecs{2}, angles(i,2)) * MatrixQuaternionRot(custom_euler_rot_vecs{3}, angles(i,3)))';

...
addpath('./src/');
inputpath='./Input/';
saveprefix='Atom_tracing_non_atom_removed_Pd1_nanoparticle';

% load recontrcution volume
rec = importdata('../3_Final_reconstruction_volume/Pd1_particle_volume.mat');
rec=double(rec);

% load tracing results
TracingResult = importdata([inputpath 'Atom_tracing_all_peaks_Pd1_particle.mat']);
...
while true
    ...
    xlabel('integrated intensity (a.u.)');
    ylabel('# atoms');
    title(sprintf('%d Pd atoms',sum(labels==2)));
...
for i=1:length(atomtype)  %if previous determined noatom is within the tight support, then change back to atom
        ...
        if tight_support(Rpos(1),Rpos(2),Rpos(3))>0
            atomtype(i) = 2;
%         else
%             atomtype(i) = 1;  %set atom outside support to NA
        end
end
labels=atomtype;
curr_model(:,labels==1)=[];
